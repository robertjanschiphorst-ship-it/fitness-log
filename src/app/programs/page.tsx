      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--text-40)] uppercase tracking-wider">{programs.length} program{programs.length !== 1 ? "s" : ""}</p>
          </div>
        ...
        {programs.length === 0 ? (
          <div className="rounded-xl border border-[var(--border)] p-10 text-center">
            <p className="text-[var(--text-40)]">No programs found.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {programs.map((p) => (
              <div key={p.id} className="group rounded-xl border border-[var(--border)] bg-[var(--card)] hover:border-orange-500/30 transition-all overflow-hidden">
                  ...
                  <div className="flex flex-1 items-start justify-between gap-4 p-4">
                    <div className="space-y-2 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${GOAL_COLOR[p.goal] ?? "text-[var(--text-50)] bg-[var(--card)] border-[var(--border)]"}`}>
                      ...
                      {p.summary && (
                        <p className="text-sm text-[var(--text-50)] leading-relaxed">{p.summary}</p>
                      )}
                    </div>
                    <div className="flex flex-col gap-2 shrink-0">
                      <Link
                        className="rounded-lg bg-[var(--input-bg)] px-3 py-1.5 text-xs font-medium text-center hover:bg-[var(--input-hover)] transition-colors"
                        href={`/programs/${p.id}`}
                      >
                        Preview
                      </Link>
